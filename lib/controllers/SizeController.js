...
class SizeController {
  ...
  resize(parentWidth, parentHeight) {
    ...
    switch (widthModificator) {
      ...
      case "contentSize":
        ...
        } else if (firstChild instanceof pixi_js.Container && firstChild.width) {
        ...
        this.layout.content.children.forEach((child) => {
          ...
          } else if (child instanceof pixi_js.Container && child.width) {
      ...
      case "static":
      default:
        finalWidth = helpers.getNumber(width, this.parentWidth);
        break;
    }
    switch (heightModificator) {
      ...
      case "contentSize":
        ...
        this.layout.content.resize(this.parentWidth, this.parentHeight);
        const { firstChild } = this.layout.content;
        if (firstChild instanceof pixi_js.Container && firstChild.height) {
        ...
        if (helpers.isItJustAText(this.layout)) {
      ...
      case "static":
      default:
        finalHeight = helpers.getNumber(height, this.parentHeight);
        break;
    }
    if (finalWidth < 0)
      finalWidth = 0;
  ...
  updateBG(finalWidth, finalHeight) {
    ...
    if (background instanceof pixi_js.NineSliceSprite || background instanceof pixi_js.TilingSprite || background instanceof pixi_js.Sprite || background instanceof pixi_js.Container) {
      if (background instanceof pixi_js.Sprite) {
      ...
      if (color && width && height) {
        ...
        if (this.bg instanceof pixi_js.Graphics) {
  ...
  updateMask() {
    ...
    if (overflow === "hidden" && width && height) {
      if (!this.overflowMask) {
        this.overflowMask = new pixi_js.Graphics();
  ...
  getAutoSizeModificator(size) {
    ...
    if (background instanceof pixi_js.Container && background.width && background.height) {
      return "background";
    }
    if (helpers.isItJustAText(this.layout) && this.layout.style.wordWrap) {
      return "innerText";
    }
    if (size === "auto") {
      return "contentSize";
   ...
   */
  set width(width) {
    this._width = helpers.getNumber(width, this.parentWidth);
    this.layout.align.resize(this.parentWidth, this.parentHeight);
  }
   ...
   */
  set height(height) {
    this._height = helpers.getNumber(height, this.parentHeight);
    this.layout.align.resize(this.parentWidth, this.parentHeight);
  }
  /**
  ...
  fitToSize(parentWidth, parentHeight) {
    ...
    const maxWidthVal = helpers.getNumber(maxWidth, parentWidth);
    const maxHeightVal = helpers.getNumber(maxHeight, parentHeight);
    const minWidthVal = helpers.getNumber(minWidth, parentWidth);
    const minHeightVal = helpers.getNumber(minHeight, parentHeight);
    if (aspectRatio === "flex") {
